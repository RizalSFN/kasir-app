                ...
                @foreach ($pagination ? $pagination : $data as $produk)
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="py-3 flex justify-center">
                            <a href="{{ route('admin.produk.status', [$produk->id_produk, $produk->status == 'ready' ? 'unready' : 'ready']) }}"
                                class="py-2 px-3 text-white mr-2 {{ $produk->status == 'ready' ? 'bg-red-400 hover:bg-red-500' : 'bg-green-400 hover:bg-green-500' }} rounded-md">
                                <i class="bi {{ $produk->status == 'ready' ? 'bi-ban' : 'bi-check-circle' }}"></i>
                            </a>
